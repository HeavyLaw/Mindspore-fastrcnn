...
def bbox_overlaps(bbox1, bbox2, mode='iou'):
    """ Calculate the iou between each bbox of bbox1 and bbox2.

    :param bbox1: ndarray, shape (n, 4)
    :param bbox2: ndarray, shape (k, 4)
    :param mode: iou (intersection over union) or iof (intersection over foreground)

    :return: ious ndarray, shape (n, k)
    """
    ...
    for i in range(bbox1.shape[0]):
        x_start = np.maximum(bbox1[i, 0], bbox2[:, 0])
        y_start = np.maximum(bbox1[i, 1], bbox2[:, 1])
        x_end = np.maximum(bbox1[i, 2], bbox2[:, 2])
        y_end = np.maximum(bbox1[i, 3], bbox2[:, 3])
